...
def compute_fill_rate_stats(matched_df):
    ...
    full_fill_rate = (matched_df['matched_quantity'] == matched_df['quantity']).mean()
    partial_fill_rate = ((matched_df['matched_quantity'] > 0) & (matched_df['matched_quantity'] < matched_df['quantity'])).mean()
    return full_fill_rate, partial_fill_rate
...
if __name__ == "__main__":
    df_orders = generate_sample_orders(n=20000, seed=42)
    ...
    plot_combined_dashboard(matched_df,rolling_window=100)
...
if __name__ == "__main__":
    df_orders = generate_sample_orders(n=20000, seed=42, qty_min=0, qty_max=200)
    matched_df = simulate_internalizer_advanced(df_orders, time_window=timedelta(minutes=10))
    matched_df['match_fill_rate'] = matched_df['matched_quantity'] / matched_df['quantity']
    matched_df['day'] = matched_df['time'].dt.date
    ...
    print(f"Full Fill Rate: {full_fill_rate:.2%}")
    print(f"Partial Fill Rate: {partial_fill_rate:.2%}")
    print("Fill Rates by Instrument: ")
    print(fill_stats_by_instr.to_string(float_format="{:.2%}".format))
    print(compute_internalization_rate_by_instrument(matched_df))
