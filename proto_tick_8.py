...
class TickPrice:
    ...
    def mid(p1, p2):
        ...
        return FloatPrice(avg_ticks, p1._market_instrument)
...
class DeltaTickPrice:
    ...
    def __repr__(self):
        return f"{self._tick.convert_to_price(self.ticks):.4f} ({self.ticks:.2f} ticks) [{self._market_instrument}]"


class FloatPrice:
    # FLoatPrice is for Quantitative purpose: VWAP, mid-price
    def __init__(self, ticks, market_instrument):
        self.ticks = float(ticks)
        self._market_instrument = market_instrument
        self._tick = TickMeta.get_tick_for_market_instrument(market_instrument)

    def to_price(self):
        return self._tick.convert_to_price(ticks=self.ticks)

    def to_tick_price(self, tick_price_cls, method=''):
        if tick_price_cls.get_instrument() != self._market_instrument._instrument:
            raise TypeError("Target TickPrice must belong to the same instrument")
        rounding_map = {
            '': lambda x: x,
            'round': round,
            'floor': math.floor,
        ...
        if method not in rounding_map:
            raise ValueError(f"Unknown rounding method: {method}")
        ticks = rounding_map[method](self.ticks)
        return tick_price_cls(ticks)
...
class Instrument:
    ...
    def register_market(self, mi):
        self.market_instruments.append(mi)
...
class MarketInstrument:
    def __init__(self, instrument: Instrument, market, tick_size):
        ...
        instrument.register_market(self)
...
if __name__ == '__main__':
    ...
    assert isinstance(midtp, FloatPrice)
    assert round(midtp.to_price(), 4) == 100.0150
    assert midtp.to_tick_price(fgbl_internal.get_tick_price_factory()).get_ticks() == 10001.5
