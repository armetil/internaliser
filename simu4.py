...
def compute_fill_rate_stats(matched_df):
    ...
    partial_fill_rate = (
                (matched_df['matched_quantity'] > 0) & (matched_df['matched_quantity'] < matched_df['quantity'])).mean()
...
def plot_combined_dashboard(matched_df, rolling_window=1000):
    ...
    num_rows = 4 + len(instruments)  # 3 base rows + 1 for fill rate trend + 1 per instrument for histograms
    ...
    fig = make_subplots(
        ...
        subplot_titles=(
            ...
            ["Match Fill Rate Over Time (Bucketed)"]
        ),
        shared_xaxes=False
    )
    ...
    # Row 1: Overall cumulative and rolling internalization rate
    ...
    # Row 2: Cumulative and rolling internalization rate by instrument
    ...
    # Last Row: Match Fill Rate Over Time (Bucketed)
    last_row = num_rows
    bucketed = matched_df.copy()
    bucketed['time_bucket'] = bucketed['time'].dt.floor('H')  # or 'D' for daily

    # Overall fill rate per time bucket
    overall_bucket = bucketed.groupby('time_bucket')['match_fill_rate'].mean().reset_index()
    fig.add_trace(go.Scatter(
        x=overall_bucket['time_bucket'],
        y=overall_bucket['match_fill_rate'],
        name='Overall Match Fill Rate',
        mode='lines+markers'
    ), row=last_row, col=1)

    # Fill rate per instrument over time
    for instr in instruments:
        sub = bucketed[bucketed['instrument'] == instr]
        instr_bucket = sub.groupby('time_bucket')['match_fill_rate'].mean().reset_index()
        fig.add_trace(go.Scatter(
            x=instr_bucket['time_bucket'],
            y=instr_bucket['match_fill_rate'],
            name=f'{instr} Match Fill Rate',
            mode='lines+markers'
        ), row=last_row, col=1)
    ...
    fig.update_layout(
        height=300 * num_rows,
        width=1000,
        title_text="Internalization Dashboard with Fill Rate Trend"
    )
    fig.show()
...
if __name__ == "__main__":
    ...
    fill_stats_html = fill_stats_df.style.format({"full_fill_rate": "{:.2%}", "partial_fill_rate": "{:.2%}"}).to_html(
        index=False)
